...
class WC_Ali_Dependencies {
	...
	public static function get_wp_notice() {
		...
		return sprintf(
			esc_html__( '%1$s is not active, as it requires WordPress version %2$s or higher. Please %3$supdate WordPress &raquo;%4$s', 'wc-ali-products-based-shipment' ),
	...
	public static function get_ali2woo_notice() {
		...
		return sprintf(
			...
			'<strong>' . WC_Ali_Products_Shipment()->plugin_name . ' is inactive </strong>',
			'<a href="' . esc_url( 'https://ali2woo.com/' ) . '">',
			'</a>',
